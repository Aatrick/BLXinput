...
l_labl = customtkinter.CTkLabel(
    app, text="Left Vibrator"
)  # place the label under the left slider
l_slider.grid(row=1, column=0, padx=50)
l_labl.grid(row=2, column=0, padx=50)

l_slider.bind(
    "<ButtonRelease-1>",
    lambda event: XInput.set_vibration(
        index, l_slider.get() / 100, r_slider.get() / 100
    ),
)
